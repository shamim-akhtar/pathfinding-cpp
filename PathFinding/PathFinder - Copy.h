...
namespace PathFinding
{
  ...
  protected:
    ...
    int IsInPQ(OpenList myList, T cell)
    {
      auto it = std::find_if(
        myList.begin(),
        myList.end(),
  ...
  protected:
    void AlgorithmSpecificImplementation(std::shared_ptr<Node<T>> cell) override
    {
      ...
      if (PathFinder<T>::IsInList(PathFinder<T>::mClosedList, (*cell).Value) == -1)
      {
        ...
        float G = 
          (*PathFinder<T>::CurrentNode).GCost + 
          (*PathFinder<T>::NodeTraversalCost)((*PathFinder<T>::CurrentNode->Location).Value, (*cell).Value);

        float H = (*PathFinder<T>::HeuristicCost)((*cell).Value, (*PathFinder<T>::Goal).Value);

        // Check if the cell is already there in the open list.
        int idOList = PathFinder<T>::IsInPQ(PathFinder<T>::mOpenList, (*cell).Value);
        if (idOList == -1)
        {
          // The cell does not exist in the open list.
          // We will add the cell to the open list.
          PathFinder::PathFinderNode<T>* pn = new PathFinder::PathFinderNode<T>(cell, PathFinder<T>::CurrentNode.get(), G, H);
          std::shared_ptr<PathFinder::PathFinderNode<T>> n(pn);
          //std::shared_ptr<PathFinder::PathFinderNode<T>> n(new PathFinder::PathFinderNode<T>(cell, G, H));
          PathFinder::mOpenList.push(n);
        }
        else
        {
          // if the cell exists in the openlist then check if the G cost 
          // is less than the one already in the list.
          float oldG = PathFinder<T>::mOpenList[idOList].GCost;
          if (G < oldG)
          {
